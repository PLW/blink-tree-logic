...
namespace mongo {
    ...
    namespace {
        ...
        bltree::Slice emptyByteSlice( "" );
        bltree::SliceParts emptyByteSliceParts( &emptyByteSlice, 1 );

        // functions for converting between BSONObj-DiskLoc pairs and strings/bltree::Slices

        /**
         * Strips the field names from a BSON object
         */
        BSONObj stripFieldNames(
            const BSONObj& obj )
        {
            BSONObjBuilder b;
         ...
         */
        IndexKeyEntry makeIndexKeyEntry(
         ...
         * BLTree cursor
         */
        class BLTreeCursor : public SortedDataInterface::Cursor {
        public:
            BLTreeCursor(
                bltree::Iterator* iterator,
                bool forward, Ordering o )
            :
                _iterator( iterator ),
                _forward( forward ),
                _isCached( false ),
            ...
            bool isEOF() const { return !_iterator->Valid(); }

            /**
             * Will only be called with other from same index as this.
             * All EOF locs should be considered equal.
             */
            bool pointsToSamePlaceAs(
                const Cursor& other) const
            {
                const BLTreeCursor* realOther = dynamic_cast<const BLTreeCursor*>( &other );
            ...
            void aboutToDeleteBucket(
                const DiskLoc& bucket)
            {
                invariant( !"aboutToDeleteBucket should never be called from BLTreeSortedDataImpl" );
            }
            ...
            void restorePosition()
            {
                ...
                if ( _savedAtEnd ) {
                    if (_forward) _iterator->SeekToLast(); else _iterator->SeekToFirst();
                    if (_iterator->Valid()) advance();
        ...
        private:
            ...
            scoped_ptr<bltree::Iterator> _iterator;
            const bool _forward;
            ...
            // Used for comparing elements in reverse iterators. Because the bltree::Iterator is
        ...
        /**
         * Custom comparator for bltree used to compare Index Entries by BSONObj and DiskLoc
         */
        class BLTreeIndexEntryComparator : public bltree::Comparator {
            public:
                BLTreeIndexEntryComparator( const Ordering& order ): _indexComparator( order ) { }
                ...
                virtual const char* Name() const {
                    // changing this means that any existing mongod instances using bltree storage
                    // engine will not be able to start up again, because the comparator's name
                    // will not match
                    return "mongodb.BLTreeIndexEntryComparator";
                }
    ...
    // BLTreeSortedDataImpl***********

    BLTreeSortedDataImpl::BLTreeSortedDataImpl(
        bltree::DB* db,
        bltree::ColumnFamilyHandle* cf,
        Ordering order )
    :
        _db( db ),
        _columnFamily( cf ),
    ...
    SortedDataBuilderInterface* BLTreeSortedDataImpl::getBulkBuilder(
    ...
    Status BLTreeSortedDataImpl::insert(
        OperationContext* txn,
        const BSONObj& key,
        const DiskLoc& loc,
        bool dupsAllowed)
    {
        BLTreeRecoveryUnit* ru = _getRecoveryUnit( txn );
        if (!dupsAllowed) {
    ...
    bool BLTreeSortedDataImpl::unindex(
        OperationContext* txn,
        const BSONObj& key,
        const DiskLoc& loc)
    {
        ...
        string dummy;
        const bltree::ReadOptions options = BLTreeEngine::readOptionsWithSnapshot( txn );
    ...
    string BLTreeSortedDataImpl::dupKeyError(
    ...
    Status BLTreeSortedDataImpl::dupKeyCheck(
        OperationContext* txn,
        const BSONObj& key,
        const DiskLoc& loc)
    {
    ...
    void BLTreeSortedDataImpl::fullValidate(
        OperationContext* txn,
        long long* numKeysOut)
    {
        // XXX: no key counts
        if ( numKeysOut ) *numKeysOut = -1;
    }

    bool BLTreeSortedDataImpl::isEmpty(
        OperationContext* txn )
    {
        // XXX doesn't use snapshot
        boost::scoped_ptr<bltree::Iterator>
            it( _db->NewIterator( bltree::ReadOptions(), _columnFamily ) );
        it->SeekToFirst();
        return it->Valid();
    }

    Status BLTreeSortedDataImpl::touch(OperationContext* txn) const
    {
        // no-op
        return Status::OK();
    }

    SortedDataInterface::Cursor* BLTreeSortedDataImpl::newCursor(
        OperationContext* txn,
        int direction) const
    {
        invariant( ( direction == 1 || direction == -1 ) && "invalid value for direction" );
        bltree::ReadOptions options = BLTreeEngine::readOptionsWithSnapshot( txn );
        return new BLTreeCursor(
                _db->NewIterator( options, _columnFamily ), direction == 1, _order );
    }

    Status BLTreeSortedDataImpl::initAsEmpty(OperationContext* txn)
    {
        // no-op
        return Status::OK();
    }

    long long BLTreeSortedDataImpl::getSpaceUsedBytes(
        OperationContext* txn ) const
    {
        boost::scoped_ptr<bltree::Iterator>
            it( _db->NewIterator( bltree::ReadOptions(), _columnFamily ) );
        it->SeekToFirst();
        const bltree::Slice rangeStart = it->key();
        it->SeekToLast();
        // This is exclusive when we would prefer it be inclusive.
        // AFB best way to get approximate size for a whole column family.
        const bltree::Slice rangeEnd = it->key();
        bltree::Range fullIndexRange( rangeStart, rangeEnd );
        uint64_t spacedUsedBytes = 0;

        // TODO BLTree specifies that this may not include recently written data.
    ...
    bltree::Comparator* BLTreeSortedDataImpl::newBLTreeComparator( const Ordering& order )
    {
        return new BLTreeIndexEntryComparator( order );
    }

    BLTreeRecoveryUnit* BLTreeSortedDataImpl::_getRecoveryUnit( OperationContext* opCtx ) const
    {
