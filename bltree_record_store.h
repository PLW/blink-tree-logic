...
namespace mongo {
    ...
    class BLTreeRecordStore : public RecordStore {
    public:
        BLTreeRecordStore( const StringData& ns,
                           bltree::DB* db,
        ...
        virtual ~BLTreeRecordStore() { }

        // name of the RecordStore implementation
        virtual const char* name() const { return "bltree"; }
        ...
        static bltreedb::Comparator* newBLTreeCollectionComparator();
    private:
         ...
         * null
         */
        bltreedb::ReadOptions _readOptions( OperationContext* opCtx = NULL ) const;

        static BLTreeRecoveryUnit* _getRecoveryUnit( OperationContext* opCtx );

        static DiskLoc _makeDiskLoc( const bltreedb::Slice& slice );
        ...
        static bltreedb::Slice _makeKey( const DiskLoc& loc );
        void _changeNumRecords(OperationContext* txn, bool insert);
        void _increaseDataSize(OperationContext* txn, int amount);

        bltreedb::DB* _db; // not owned
        bltreedb::ColumnFamilyHandle* _columnFamily; // not owned
        bltreedb::ColumnFamilyHandle* _metadataColumnFamily; // not owned
